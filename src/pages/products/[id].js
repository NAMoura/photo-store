  ...
  return (
    <div className="container lg:max-w-screen-lg mx-auto py-12 px-6">
      <div className="flex flex-col md:flex-row justify-between items-center">
        ...
        <div className="w-full flex-1 max-w-md border border-opacicy-50 rounded-md shadow-lg p-6 bg-white">
          ...
          <button onClick={onAddToCart} className="w-full mt-4 border-lime-500 py-2 px-6 bg-lime-500 hover:bg-lime-600 hover:border-lime-600 focus:ring-4 focus:ring-opacity-50 focus:ring-lime-500 text-white disabled:opacity-50 disabled:cursor-not-allowed rounded">
            Add to cart
          </button>
        </div>
      </div>
...
export async function getStaticProps({ params }) {
  const inventory = await stripe.products.list({
    //default_price is just an ID, this expands it's properties
